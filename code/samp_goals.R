...
#unique(dat$SEASON) you season NOT year
dat %>%
...
recruits %>% 
  na.omit() %>%
  mutate( value = crab_power(a = 0.10, b = sd_percentR, signif = 0.05, power = 0.85))

# writing a function
crab_power <- function(a, b, signif, power){
  step1 <- pwr.t.test(d=(a/b), sig.level = signif, power = power, type = "one.sample", alternative = "two.sided")
  step1$n
}
crab_power(0.10, recruits$sd_percentR, 0.05, 0.85)

apply(recruits$sd_percentR, 1, crab_power(a =0.10, b = sd_percentR, signif = 0.05, power = 0.85))

by_row(recruits, crab_power, a=0.10, signif = 0.05, power = 0.85, .collate = "list")
